...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [e,minElArea] = computeRelErH1DispInfinitePlateWithHoleIGAPlateInMembraneAction(p,q,Xi,Eta,CP,isNURBS,parameters,Radius,T,dHat,error,outMsg)
...
% action problem which models the one quarter of an infinite plate with a 
% circular hole under a tensional loading at +-infty.
%
%          Input :
...
%         Radius : The radius of the hole in the middle of the plate
%              T : The magnitude of the tensional loading applied at
%                  x=+-infty
        ...
        if Eta(j)-Eta(j+1)~=0 && Xi(i)-Xi(i+1)~=0
            ...
            for l=1:error.etaNGP
                for k=1:error.xiNGP
                    ...
                    % Compute some auxiliary parameters
                    mue = parameters.E/(1+parameters.nue)/2;
                    ...
                    if strcmp(error.component,'x')
                        ...
                        resExactL2 =  -T*Radius*(r*(kappa+1)*cos(theta)/...
                            Radius+2*Radius*((1+kappa)*cos(theta)+cos(3*theta))/...
                            r-2*Radius^3*cos(3*theta)/r^3)/mue/8;

                        % The H1-part of the norm
                        resExactH1 = zeros(2,1);
                        resExactH1(1,1) = T*(10*Radius^2*r^2*cos(theta)^2+18*cos(theta)^2*...
                            Radius^2*r^2*parameters.nue-24*cos(theta)^2*Radius^4-...
                            24*cos(theta)^2*Radius^4*parameters.nue-16*cos(theta)^4*Radius^2*r^2-...
                            16*cos(theta)^4*Radius^2*r^2*parameters.nue+24*cos(theta)^4*Radius^4+...
                            24*cos(theta)^4*Radius^4*parameters.nue+2*r^4+Radius^2*r^2-...
                            3*Radius^2*r^2*parameters.nue+3*Radius^4+3*Radius^4*parameters.nue)/...
                            r^4/parameters.E/2;
                        resExactH1(2,1) = T*Radius^2*sin(theta)*cos(theta)*(-8*cos(theta)^2*r^2*parameters.nue+...
                            12*cos(theta)^2*Radius^2+12*cos(theta)^2*Radius^2*parameters.nue-...
                            8*cos(theta)^2*r^2-r^2-6*Radius^2*parameters.nue+3*r^2*parameters.nue-6*Radius^2)/...
                            r^4/parameters.E;
                    ...
                    elseif strcmp(error.component,'y')
                        ...
                        % The L2-part of the norm
                        resExactL2 = -T*Radius*(r*(kappa-3)*sin(theta)/Radius+...
                            2*Radius*((1-kappa)*sin(theta)+sin(3*theta))/r-...
                            2*Radius^3*sin(3*theta)/r^3)/mue/8;

                        % The H1-part of the norm
                        resExactH1 = zeros(2,1);
                        resExactH1(1,1) = T*Radius^2*sin(theta)*cos(theta)*(-8*cos(theta)^2*r^2*parameters.nue+...
                            12*cos(theta)^2*Radius^2+12*cos(theta)^2*Radius^2*parameters.nue-8*cos(theta)^2*r^2+...
                            7*r^2+3*r^2*parameters.nue-6*Radius^2*parameters.nue-6*Radius^2)/r^4/parameters.E;
                        resExactH1(2,1) = -T*(-24*cos(theta)^2*Radius^4*parameters.nue+...
                            10*Radius^2*r^2*cos(theta)^2*parameters.nue+18*Radius^2*r^2*cos(theta)^2-...
                            24*cos(theta)^2*Radius^4-16*cos(theta)^4*Radius^2*r^2-...
                            16*cos(theta)^4*Radius^2*r^2*parameters.nue+24*cos(theta)^4*Radius^4*parameters.nue+...
                            24*cos(theta)^4*Radius^4-3*Radius^2*r^2+Radius^2*r^2*parameters.nue+3*Radius^4+...
                            3*Radius^4*parameters.nue+2*parameters.nue*r^4)/r^4/parameters.E/2;
                    ...
                    elseif strcmp(error.component,'tensor')
                        ...
                        % The L2-part of the norm
                        dxExact = -T*Radius*(r*(kappa+1)*cos(theta)/Radius+...
                            2*Radius*((1+kappa)*cos(theta)+cos(3*theta))/...
                            r-2*Radius^3*cos(3*theta)/r^3)/mue/8;
                        dyExact = -T*Radius*(r*(kappa-3)*sin(theta)/Radius+...
                            2*Radius*((1-kappa)*sin(theta)+sin(3*theta))/...
                            r-2*Radius^3*sin(3*theta)/r^3)/mue/8;
                        ...
                        resExactH1(1,1) = T*(10*Radius^2*r^2*cos(theta)^2+18*cos(theta)^2*Radius^2*r^2*parameters.nue-...
                            24*cos(theta)^2*Radius^4-24*cos(theta)^2*Radius^4*parameters.nue-16*cos(theta)^4*Radius^2*r^2-...
                            16*cos(theta)^4*Radius^2*r^2*parameters.nue+24*cos(theta)^4*Radius^4+...
                            24*cos(theta)^4*Radius^4*parameters.nue+2*r^4+Radius^2*r^2-3*Radius^2*r^2*parameters.nue+...
                            3*Radius^4+3*Radius^4*parameters.nue)/r^4/parameters.E/2;
                        resExactH1(1,2) = T*Radius^2*sin(theta)*cos(theta)*(-8*cos(theta)^2*r^2*parameters.nue+...
                            12*cos(theta)^2*Radius^2+12*cos(theta)^2*Radius^2*parameters.nue-8*cos(theta)^2*r^2-...
                            r^2-6*Radius^2*parameters.nue+3*r^2*parameters.nue-6*Radius^2)/r^4/parameters.E;
                        resExactH1(2,1) = T*Radius^2*sin(theta)*cos(theta)*(-8*cos(theta)^2*r^2*parameters.nue+...
                            12*cos(theta)^2*Radius^2+12*cos(theta)^2*Radius^2*parameters.nue-8*cos(theta)^2*r^2+...
                            7*r^2+3*r^2*parameters.nue-6*Radius^2*parameters.nue-6*Radius^2)/r^4/parameters.E;
                        resExactH1(2,2) = -T*(-24*cos(theta)^2*Radius^4*parameters.nue+...
                            10*Radius^2*r^2*cos(theta)^2*parameters.nue+18*Radius^2*r^2*cos(theta)^2-...
                            24*cos(theta)^2*Radius^4-16*cos(theta)^4*Radius^2*r^2-16*cos(theta)^4*Radius^2*r^2*parameters.nue+...
                            24*cos(theta)^4*Radius^4*parameters.nue+24*cos(theta)^4*Radius^4-3*Radius^2*r^2+...
                            Radius^2*r^2*parameters.nue+3*Radius^4+3*Radius^4*parameters.nue+2*parameters.nue*r^4)/...
                            r^4/parameters.E/2;
