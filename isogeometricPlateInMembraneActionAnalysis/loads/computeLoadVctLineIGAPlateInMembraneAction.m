...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Fl = computeLoadVctLineIGAPlateInMembraneAction(FlOld,xib,etab,p,q,Xi,Eta,CP,isNURBS,FAmp,direction,int,outMsg)
...
%             1 = x, 2 = y for f(s) -> e.g. self-weight
%             3 = x, 4 = y for f(x,y) -> e.g. wind, snow
%             5 = parallel, 6 = +90  to the edge
%             xiEta: dir to integrate: 
%             1 = xi, 2 = eta. determined from xib,etab
%             xys: 
%             1 = integrate ds   2 = integrate dx,dy  determined 
...
% Find the span in eta-direction where to apply the load
            ...
            for cEta = 1:etaNGP
                for cXi = 1:xiNGP
                    ...
                    if isscalar(xib)==0
                        ...
                        % Issue quadrature weight in xi-direction
                        xigw = xiGW(cXi);
                    end
                    ...
                    if isscalar(etab)==0
                        ...
                        % issue quadrature weight in eta-direction
                        etagw = etaGW(cEta);
                    end
                    ...
                    %% 3i.7. Compute the element load vector
                    if direction==1||direction==2
                        Fel(:,:,direction) = FAmplitude*Rds(:,:)*GW*map;
                    elseif direction==3
                        Fel(:,:,1) = FAmplitude*abs(Rds(:,:,2))*GW*map;
                    elseif direction==4
                        Fel(:,:,2) = FAmplitude*abs(Rds(:,:,1))*GW*map;
                    elseif direction==5
                        Fel(:,:,1) = FAmplitude*Rds(:,:,1)*GW*map;
                        Fel(:,:,2) = FAmplitude*Rds(:,:,2)*GW*map;
                    elseif direction==6
                        Fel(:,:,1) = -FAmplitude*Rds(:,:,2)*GW*map;
                        Fel(:,:,2) =  FAmplitude*Rds(:,:,1)*GW*map;
                    end
                    
                    %% 3i.8. Assemble the element load vector to the global load vector
                    F(i-p:i,j-q:j,:) = F(i-p:i,j-q:j,:) + Fel(:,:,:);
                end  
            end
        end
    end
