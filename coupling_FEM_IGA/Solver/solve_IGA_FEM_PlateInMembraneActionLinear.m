...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [dHat,FComplete,minElSize,K] = solve_IGA_FEM_PlateInMembraneActionLinear...
    (Beta,XiB,p,Xi,q,Eta,CP,isNURBS,parameters,F_IGA,bodyForces,homDOFsIGA,inhomDOFs,...
    valuesInhomDOFs,int,outMsg,...
    strMsh,homDOFsFEM,inhomDOFsFEM,valuesInhomDOFsFEM,...
    NBC,IBC,F_FEM,bodyForcesFEM,analysis,parametersFEM,nLinearAnalysisFEM,strDynamics,intFEM,...
...
%           Input :
%             Beta: Penalty Term
%              XiB: fixed knot on the interface
...
% Get number of DOFs for the IGA side
noDOFsIGA = 2*noCPs;

% Get the number of DOFs for the FEM side
noDOFsFEM = 2*length(strMsh.nodes(:,1));

% Initialize output array
dHat = zeros(noDOFsIGA+nDOFsFEM,1);
...
% Set up the global array for the Dirichlet DOFs
homDOFs = [homDOFsFEM homDOFsIGA+noDOFsFEM];

% Get a sequencial numbering of the unconstained DOFs into a vector
freeDOFs = zeros(length(F_IGA)+length(F_FEM),1);
for i=1:length(F_IGA)+length(F_FEM)
    freeDOFs(i,1) = i;
end
...
%% 1. Solve the linear equation system
[dHat,~,~,FComplete,minElSize,K] = solve_IGA_FEM_LinearSystem(Beta,dHatSaved, ...
    dHatDotSaved,dHatDDotSaved,XiB,BSplinePatch,F_IGA,bodyForces,parameters,...
    dHat,dHatDot,@computeStiffMtxAndLoadVctIGA_FEMPlateInMembraneActionLinear,...
    @computeModifiedLinearSystem,...
    DOFNumbering,freeDOFs,homDOFsIGA,inhomDOFs,valuesInhomDOFs,transientAnalysis,...
    t,nLinearAnalysis,int,outMsg,...
    uSaved, ...
    uDotSaved,uDDotSaved,strMsh,analysis,IBC,F_FEM,bodyForcesFEM,parametersFEM,uDot, ...
    DOFNumberingFEM,homDOFsFEM,inhomDOFsFEM,valuesInhomDOFsFEM,...
    strDynamics,nLinearAnalysisFEM);
