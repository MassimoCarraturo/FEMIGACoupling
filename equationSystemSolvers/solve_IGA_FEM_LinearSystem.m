...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [u,noNewtonIter,hasConverged,FComplete,minElSize] = solve_IGA_FEM_LinearSystem(uSaved, ...
    ...
    uSavedFEM, ...
    uDotSavedFEM,uDDotSavedFEM,meshFEM,analysisFEM,FFEM,bodyForcesFEM,parametersFEM,uFEM,uDotFEM, ...
    computeLinearMatricesFEM,DOFNumberingFEM,freeDOFsFEM,homDOFsFEM,inhomDOFsFEM, ...
    valuesInhomDOFsFEM,strDynamics,nLinearAnalysisFEM,intFEM,outMsgFEM)
...
[KIGA,RHSIGA,minElSizeIGA] = computeLinearMatrices(u,uSaved,uDot,uDotSaved, ...
    DOFNumbering,BSplinePatch,F,transientAnalysis,t,parameters,...
    bodyForces,int);

[KFEM,RHSFEM,minElSizeFEM] = computeLinearMatrices(uFEM,uSavedFEM,uDotFEM,uDotSavedFEM, ...
    DOFNumberingFEM,meshFEM,analysisFEM,FFEM,bodyForcesFEM,strDynamics,parametersFEM,intFEM,outMsgFEM);

K=blkdiag(KFEM,KIGA);

%% 2. Update the right-hand side vector if inhomogeneous Dirichlet boundary conditions are encountered
if norm(valuesInhomDOFs(inhomDOFs)) ~= 0
    RHSIGA = RHSIGA - KIGA(:,inhomDOFs)*valuesInhomDOFs(inhomDOFs);
end
%FEM
if norm(valuesInhomDOFsFEM) ~= 0
    RHSFEM = RHSFEM - KFEM(:,inhomDOFsFEM)*valuesInhomDOFsFEM';
end
