...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [e,minElArea] = computeRelErrorL2InfinitePlateWithHoleIGAPlateInMembraneAction(p,q,Xi,Eta,CP,isNURBS,parameters,Radius,T,dHat,error,outMsg)
...
% which models the one quarter of an infinite plate with a circular hole
% under a tensional loading at +-infty
%
%          Input :
...
if strcmp(outMsg,'outputEnabled')
    ...
    fprintf('problem of a infinite plate with a circular hole in membrane action\n'); 
    fprintf('subject to tensional loading in x=+-infty has been initiated\n\n');
                ...
                for k=1:error.xiNGP
                    ...
                    if strcmp(error.resultant,'displacement')
                        ...
                        if strcmp(error.component,'x')
                            ...
                            resExact =  -T*Radius*(r*(kappa+1)*cos(theta)/Radius+2*Radius*((1+kappa)*cos(theta)+cos(3*theta))/r-2*Radius^3*cos(3*theta)/r^3)/mue/8;
                        ...
                        elseif strcmp(error.component,'y')
                            % Compute the analytical resultant on the Gauss
                            % Point and on the Cartesian space
                            resExact = -T*Radius*(r*(kappa-3)*sin(theta)/Radius+2*Radius*((1-kappa)*sin(theta)+sin(3*theta))/r-2*Radius^3*sin(3*theta)/r^3)/mue/8;
                        ...
                        elseif strcmp(error.component,'tensor')
                            ...
                            dxExact = -T*Radius*(r*(kappa+1)*cos(theta)/Radius+2*Radius*((1+kappa)*cos(theta)+cos(3*theta))/r-2*Radius^3*cos(3*theta)/r^3)/mue/8;
                            dyExact = -T*Radius*(r*(kappa-3)*sin(theta)/Radius+2*Radius*((1-kappa)*sin(theta)+sin(3*theta))/r-2*Radius^3*sin(3*theta)/r^3)/mue/8;
                            resExact = zeros(2,1);
                            resExact(1,1) = dxExact;
                            resExact(2,1) = dyExact;
                    ...
                    elseif strcmp(error.resultant,'stress')
                        ...
                        % curvilinear space
                        pr = t*(1-rr^2) + t*(1-4*rr^2+3*rr^4)*cos(a);
                        pt = t*(1+rr^2) - t*(1+3*rr^4)*cos(a);
                        prt = -t*(1+2*rr^2-3*rr^4)*sin(a);
...
if strcmp(outMsg,'outputEnabled')
    ...
    fprintf('_____________________Error Computation Ended_______________________\n');
    fprintf('###################################################################\n\n\n');
end
