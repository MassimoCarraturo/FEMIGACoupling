...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [e,minElArea] = computeRelErrorL2PressureVesselIGAPlateInMembraneAction(p,q,Xi,Eta,CP,isNURBS,parameters,internalRadius,externalRadius,pAmp,dHat,error,outMsg)
...
% which models a pressure vessel with a constant internal pressure.
%
%          Input :
...
if strcmp(outMsg,'outputEnabled')
    ...
    fprintf('problem of a pressure vessel-like plate in membrane action subject\n');
    fprintf('to uniform internal pressure has been initiated\n\n');
        ...
        if Eta(j)-Eta(j+1)~=0 && Xi(i)-Xi(i+1)~=0
            ...
            for l=1:error.etaNGP
                for k=1:error.xiNGP
                    ...
                    if strcmp(error.resultant,'displacement')
                        % Compute the analytical displacement components in
                        % the curvilinear basis
                        ur = (1+parameters.nue)*internalRadius^2*pAmp*((1-2*parameters.nue)*r/...
                            (1+parameters.nue)+externalRadius^2/r)/parameters.E/(externalRadius^2-internalRadius^2);
                        ut = 0;
                        
                        % Compute the numerical displacement vector over
                        % the Cartesian basis
                        ...
                        if strcmp(error.component,'x')
                            % Compute the analytical displacement component
                            % in the Cartesian basis
                            resExact = ur*cos(theta)-r*ut*sin(theta);
                        ...
                        elseif strcmp(error.component,'y')
                            % Compute the analytical displacement component
                            % in the Cartesian basis
                            resExact = ur*sin(theta)+r*ut*cos(theta);
                            
                            % Compute the numerical resultant on the Gauss
                            % Point
                            resNumerical = d(2,1);
                        elseif strcmp(error.component,'tensor')
                            % Compute the analytical displacement vector
                            % in the Cartesian basis
                            resExact = zeros(2,1);
                            resExact(1,1) = ur*cos(theta)-r*ut*sin(theta);
                            resExact(2,1) = ur*sin(theta)+r*ut*cos(theta);
                            
                            % Compute the numerical displacement vector on 
                            % the Gauss Point
                            resNumerical = d;
                        end
                    elseif strcmp(error.resultant,'strain')
                    ...
                    elseif strcmp(error.resultant,'stress')
                        ...
                        % curvilinear space
                        pr = pAmp*internalRadius^2*(r^2-externalRadius^2)/r^2/(externalRadius^2-internalRadius^2);
                        pt = pAmp*internalRadius^2*(r^2+externalRadius^2)/r^2/(externalRadius^2-internalRadius^2);
                        prt = 0;
