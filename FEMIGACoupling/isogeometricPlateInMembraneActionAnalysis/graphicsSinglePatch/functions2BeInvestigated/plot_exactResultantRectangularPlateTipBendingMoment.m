...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plot_exact_resultant_rectangular_membrane_tip_bending_moment(membrane,l,h,ql,npoints,graph,clr)
%% Function documentation
%
% Plots the exact resultant (displacement,strain,stress) for a rectangular
% plate fixed at one edges subject to tip bending moment at the free edge
%
%        Input : 
    ...
    if vcoup(1)==vcoup(2)
        ...
        if graph.resultant==1
            if graph.component==1
                resultant(i,1) = -2*ql*curve_new(1)*curve_new(2)/E/h/th;
            elseif graph.component==2
                resultant(i,1) = ql*curve_new(1)^2/E/h/th; 
            elseif graph.component==3
                displ = zeros(2,1);
                displ(1,1) = -2*ql*curve_new(1)*curve_new(2)/E/h/th;
                displ(2,1) = ql*curve_new(1)^2/E/h/th; 
                resultant(i,1) = norm(displ);
            end
        elseif graph.resultant==2
            if graph.component==1
                resultant(i,1) = -2*ql*curve_new(2)/E/h/th;
           elseif graph.component==2
                resultant(i,1) = -2*ql*curve_new(2)*nue/E/h/th;
            elseif graph.component==3
                resultant(i,1) = 0;
            end
        elseif graph.resultant==3
            if graph.component==1
                resultant(i,1) = -2*ql*curve_new(2)/h;
            elseif graph.component==2
                resultant(i,1) = 0;
            elseif graph.component==3
                resultant(i,1) = 0;
