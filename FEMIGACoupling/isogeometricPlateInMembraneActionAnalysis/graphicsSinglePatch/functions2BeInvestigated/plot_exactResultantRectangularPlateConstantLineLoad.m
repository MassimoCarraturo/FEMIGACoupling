...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function plot_exact_resultant_rectangular_membrane_constant_line_load(membrane,l,h,ql,npoints,graph,clr)
%% Function documentation
%
% Plots the exact resultant (displacement,strain,stress) for a rectangular
% plate fixed at two opposite edges subject to uniform pressure load at
% another edge
%
%        Input : 
    ...
    if vcoup(1)==vcoup(2)
        ...
        if graph.resultant==1
            if graph.component==1
                resultant(i,1) = ql*curve_new(1)*(5*h^3*nue+3*h^2*(2+5*nue)*curve_new(2)-5*curve_new(2)*(3*l^2-4*curve_new(1)^2+4*(2+nue)*curve_new(2)^2))/10/E/h^3/th;
            elseif graph.component==2
                resultant(i,1) = ql*curve_new(2)*(10*h^3*nue+3*h^2*(2+5*nue)*curve_new(2)-5*curve_new(2)*(3*l^2+2*(-6*curve_new(1)^2+(2+nue)*curve_new(2)^2)))/20/E/h^3/th;
            end
        elseif graph.resultant==2
            if graph.component==1
                resultant(i,1) = ql*(5*h^3*nue+3*h^2*(2+5*nue)*curve_new(2)-5*curve_new(2)*(3*l^2+4*(-3*curve_new(1)^2+(2+nue)*curve_new(2)^2)))/10/E/h^3/th;
            elseif graph.component==2
                resultant(i,1) = -ql*(5*h^3+3*h^2*(5+2*nue)*curve_new(2)-5*curve_new(2)*(3*l^2*nue+4*(-3*nue*curve_new(1)^2+curve_new(2)^2+2*nue*curve_new(2)^2)))/10/E/h^3/th;
            elseif graph.component==3
                resultant(i,1) = (1+nue)*ql*curve_new(1)*(3*h^2-12*curve_new(2)^2)/2/E/h^3/th;
            end
        elseif graph.resultant==3
            if graph.component==1
                resultant(i,1) = -3*ql*(l^2-4*curve_new(1)^2)*curve_new(2)/2/h^3+ql*(3*h^2-20*curve_new(2)^2)*curve_new(2)/5/h^3;
            elseif graph.component==2
                resultant(i,1) = -ql*(h^3+3*h^2*curve_new(2)-4*curve_new(2)^3)/2/h^3;
            elseif graph.component==3
                resultant(i,1) = ql*(3*h^2-12*curve_new(2)^2)*curve_new(1)/2/h^3;
            end
        end
